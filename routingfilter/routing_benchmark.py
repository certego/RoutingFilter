...
class RoutingBenchMark():
    ...
    def test1_EQUALS_no_key_match(self):
        ...
        self.benchmark_event_1_equals = load_test_data("benchmark_event_1_equals")
        self.routing.load_from_dicts([rule])
        start_time = datetime.now()
        # Sending 100 messages to the routing
        for i in range(MAX_EVENT):
            self.routing.match(self.benchmark_event_1_equals)
    ...
    def test2_EQUALS_key_exists(self):
        ...
        self.benchmark_event_1_equals.update({"wheel_model": "test"})
        self.routing.load_from_dicts([rule])
        start_time = datetime.now()
        # Sending 100 messages to the routing
        for i in range(MAX_EVENT):
    ...
    def test4_EQUALS_list_values_message(self):
        ...
        self.benchmark_event_1_equals = load_test_data("benchmark_event_1_equals")
        self.benchmark_event_1_equals.update({"wheel_model": []})
        for i in range(MAX_LIST_VALUES):
            self.benchmark_event_1_equals["wheel_model"].append("test-" + str(i))
        start_time = datetime.now()
        # Sending 100 messages to the routing
        for i in range(MAX_EVENT):
            self.routing.match(self.benchmark_event_1_equals)
